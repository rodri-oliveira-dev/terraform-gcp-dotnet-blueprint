module "worker" {
  ...
  location        = var.region
  description     = "Development request-serving worker for authenticated Pub/Sub push delivery."
...
module "api" {
  ...
  location        = var.region
  description     = "Development .NET API. Invocation remains authenticated and non-public by default."
...
resource "google_cloud_scheduler_job" "batch" {
  ...
  description      = "Starts the development Cloud Run batch job through the Cloud Run Admin API."

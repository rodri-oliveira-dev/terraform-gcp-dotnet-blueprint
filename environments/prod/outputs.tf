...
output "network" {
  description = "Production VPC and subnet identifiers used by Direct VPC egress."
  value = {
...
output "api_uri" {
  description = "Production API URI when workloads are enabled. Invocation is not made public by this root."
  value       = var.enable_workloads ? module.api[0].uri : null
}

output "worker_uri" {
  description = "Production Pub/Sub worker URI when workloads are enabled."
...
output "batch_execution_uri" {
  description = "Cloud Run Admin API execution URI for the production batch job when enabled."

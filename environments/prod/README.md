...
This root composes the reusable modules into the production counterpart of `environments/dev` for issue #8.

It intentionally keeps the same architectural boundaries as development while applying explicit production-oriented sizing, availability, retry, and state-isolation policy.
...
3. Set `enable_workloads = true`, review the complete production plan, obtain the required approval, and apply the workloads.

This avoids using `terraform -target` as the normal deployment model and prevents Terraform variables/source from carrying application secret payloads.
...
Repository CI validates this root with the committed provider lock file and `terraform init -backend=false`, followed by Terraform validation, formatting, TFLint, and Trivy. CI must not create production resources.
